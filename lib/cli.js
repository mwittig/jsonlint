...
var options = require("commander")
  .arguments('[file]')
  .description('validates the given file or reads from stdin if file is omitted')
  .version(require("../package").version, '-v, --version')
  .option('-s, --sort-keys', 'sort object keys')
  .option('-i, --in-place', 'overwrite the file')
  .option('-t, --indent <char>', 'character(s) to use for indentation (two spaces if omitted)')
  .option('-c, --compact', 'compact error display')
  .option('-V, --validate <file>','a JSON schema to use for validation')
  .option('-e, --environment <schema>','which specification of JSON Schema the validation file uses (json-schema-draft-03 if omitted)')
  .option('-q, --quiet', 'do not print the parsed json to STDOUT')
  .option('-p, --pretty-print','force pretty printing even if invalid')
  .action(function (file, options) {
    options.file = file;
    options.sort = options.sortKeys;
    options.env = options.environment;
    options.forcePrettyPrint = options.prettyPrint;
    options.inplace = options.inPlace;
    if (options.indent === undefined) {
      options.indent =  '  ';
    }
    if (options.env === undefined) {
      options.env =  'json-schema-draft-03';
    }
  })
  .parse(process.argv);
